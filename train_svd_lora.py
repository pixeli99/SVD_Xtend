...
from diffusers.utils import check_min_version, deprecate, is_wandb_available, load_image, convert_state_dict_to_diffusers
...
class DummyDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if not os.path.exists(folder_path):
            return self.__getitem__((idx+1) % len(self)) # simply skip the error data
        frames = os.listdir(folder_path)
        # Sort the frames by name
        frames.sort()

        # Ensure the selected folder has at least `sample_frames`` frames
        if len(frames) < self.sample_frames + 10:
            # raise ValueError(
            #     f"The selected folder '{chosen_folder}' contains fewer than `{self.sample_frames}` frames.")
            print(f"The selected folder '{chosen_folder}' contains fewer than `{self.sample_frames+10}` frames.")
            return self.__getitem__((idx+1) % len(self)) # simply skip the error data

        # Randomly select a start index for frame sequence
        start_idx = random.randint(5, len(frames) - self.sample_frames - 5)
...
def main():
    ...
    with open("all_unet_params.txt",'w',encoding='utf8') as file:
    ...
    def encode_image(pixel_values):
        # pixel: [-1, 1] @NOTE bug has been fixed.
    ...
    def _get_add_time_ids(
        ...
        # @NOTE use the following if use single GPU.
    ...
    if accelerator.is_main_process:
        ...
        with torch.autocast(
            ...
            for val_img_idx in range(args.num_validation_images):
                ...
                out_file = os.path.join(
                    val_save_dir,
                    f"step_final_val_img_{val_img_idx}.mp4",
                )
